...
SYSTEME = (
    ...
    "repository root. Exceptions: the user gives an explicit path, or you "
    "are modifying an EXISTING project (then work where it already lives - "
    "e.g. a project already under outputs/ stays under outputs/).\n"
    ...
    "executer(args, config) -> str. Plugins load at startup: after creating "
    "one, tell the user to /restart so you gain the ability. IMPORTANT: when "
    ...
    "wttr.in for weather), then remind them to /restart.\n"
...
AIDE_LOGICIEL = (
    ...
    "- /plugins : list the extra tools loaded from the plugins/ folder (name, "
    "description, file, and whether they require confirmation). To ADD an "
    "ability, the user can drop a plugin file there — or simply ask YOU to "
    "create one (you write plugins/<name>.py, then /restart loads it).\n"
